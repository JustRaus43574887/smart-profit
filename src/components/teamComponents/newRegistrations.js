...
function NewRegistrations({ skip, search, setLength }) {
  ...
  useEffect(() => {
    (async () => {
      const result = await request(
        `/get-new-partners?limit=25&skip=${skip}`,
        "GET",
        null,
        {
        ...
        <div className="table-responsive" style={{ marginTop: 50 }}>
          <table className="general_table" style={{ marginTop: 0 }}>
            <tbody>
              ...
              {data.map((item, index) => (
                <TableItem item={item} key={index} t={t} search={search} />
              ))}
            </tbody>
          </table>
        </div>
    ...
    >
          ...
          {item && (
            <>
              ...
              <div className="popover__wrapper">
                ...
                <div className="popover__content" style={{ height: 150 }}>
      ...
      <td className="child_row">
        <p className="register_text">
          {item && new Date(item.register_date * 1000).toLocaleDateString()}
        </p>
      </td>
    </tr>
  );
